...
const orderDao = {
    ...
    ordercord(req,resp){
        return new Promise(function (resolve,reject) {
            db.connect("SELECT IFNULL(SUM(go.o_price),0) AS o_price,IFNULL(SUM(CEILING(go.o_price/go.total)),0) AS total_price,(SELECT COUNT(is_pay) FROM goodsorder WHERE is_pay!=0 ) AS cpay,(SELECT COUNT(is_pay) FROM goodsorder WHERE is_pay=0 ) AS copay,(SELECT SUM(go.o_price) FROM goodsorder go WHERE go.isClosed=0) AS sprice\n" +
                "FROM goodsorder go\n" +
    ...
    ordercordlastyear(req,resp){
        return new Promise(function (resolve,reject) {
            db.connect("SELECT IFNULL(SUM(go.o_price),0) AS o_price,IFNULL(SUM(CEILING(go.o_price/go.total)),0) AS total_price,(SELECT COUNT(is_pay) FROM goodsorder WHERE is_pay!=0 AND YEAR(createtime)=YEAR(DATE_SUB(NOW(),INTERVAL 1 YEAR))) AS cpay,(SELECT COUNT(is_pay) FROM goodsorder WHERE is_pay=0 AND YEAR(createtime)=YEAR(DATE_SUB(NOW(),INTERVAL 1 YEAR))) AS copay,(SELECT IFNULL(SUM(go.o_price),0) FROM goodsorder go WHERE go.isClosed=0 AND YEAR(createtime)=YEAR(DATE_SUB(NOW(),INTERVAL 1 YEAR))) AS sprice\n" +
                "FROM goodsorder go\n" +
